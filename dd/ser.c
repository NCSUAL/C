...
char *plcmsg = "당신은 경찰입니다. 밤에 마피아가 누군지 알 수 있는 기회가 부여됩니다.\n";
char *ctzmsg = "당신은 시민입니다. 마피아를 찾아주세요!\n";
char *morning = "\n\t낮이 밝았습니다. 마피아가 누구일지 추리해주세요!\n";
char *nightmsg = "\t누구를 죽일 지 결정해주세요: ";
char *vote = "------------------------------------------------------\n\t\t투표를 시작합니다.\n\t마피아라고 생각하는 사용자의 번호를 입력하세요!\n";
char *voteresult = "------------------------------------------------------\n\t 가장 많은 투표를 받은 사람은.. ";
char *votenotice = "다음 사용자들을 제외하고 투표하세요)";
char *userdie = "\t사용자는 죽었습니다.. 투표에서 0을 눌러주세요\n";
char *victory = "\t이 사람은 마피아입니다.\n\n";
char *nvictory = "\t이 사람은 마피아가 아닙니다.\n";
...
void *chat_thread(void *arg) {
    ...
    while (tv.tv_sec > 0) {
        FD_ZERO(&read_fds);
        FD_SET(clisock_list[idx], &read_fds);

        int result = select(clisock_list[idx] + 1, &read_fds, NULL, NULL, &tv);
        if (result < 0) {
            perror("select error");
            break;
        } else if (result == 0) {
            printf("1분이 지났습니다. 클라이언트 %d 연결을 유지합니다.\n", clisock_list[idx]);
            break;
        }

        if (FD_ISSET(clisock_list[idx], &read_fds)) {
            int nbyte = recv(clisock_list[idx], buf, MAXLINE, 0);
            if (nbyte <= 0) {
                perror("recv error");
                break;
            }
            ...
            buf[nbyte] = 0;
            printf("Received from %d: %s\n", clisock_list[idx], buf);

            // 메시지 처리 로직 (예: 다른 클라이언트에게 메시지 전송)
        }
    }
      ...
      if (num_chat == 5) {         // 게임시작
         ...
         while (fgets(buf, sizeof(buf), stdin) != NULL) {      //사회자(서버)가 start를 입력하면 시작
            ...
            if (strstr(buf, start_command1) != NULL) {   //buf가 start이면
                ...
                *idx = i;
                pthread_t tid;
                pthread_create(&tid, NULL, chat_thread, idx);
                pthread_detach(tid);
            	}
			   ...
			   while (1) {
                  ...
                  if (max == mafia + 1) { // 마피아가 맞는 경우
                     ...
                     if (max == police + 1) {   // 경찰이 죽음
                        ...
                        printf("        ?    ? .\n");
                        for (i = 0; i < num_chat; i++)
                           send(clisock_list[i], policedie, strlen(policedie), 0);// 아니라는 말과 함께 밤으로 넘어간다.
                     }
                     else {          // 시민이 죽음
                        numctz--;
                        printf("\t남은 시민 수: %d명\n\n", numctz);
                        ...
                        for (i = 0; i < num_chat; i++)
                           send(clisock_list[i], incorrect, strlen(incorrect), 0);// 아니라는 말과 함께 밤으로 넘어간다.
                  ...
                  for (i = 0; i < num_chat; i++) {
                     if (i == mafia) {   // 마피아 차례
                        ...
                        while (1) {
                           errno = 0;
                           1 == 1; //nbyte = recv(clisock_list[i], vvote, MAXLINE, 0);
                           nbyte = recv(clisock_list[mafia], target, MAXLINE, 0);// 메시지 청취
                           if (nbyte > 0) {
                              indexx++; //
                              break;   //while
                           }
